...
int main(int argc, char** argv)
{
  ...
  const int M = 8192;//32768; // number of rows 
  const int N = 2048;//8192; // number of columns
  ...
  int numThreads = 256;
  int count = (M/numThreads)+1;
  transpose_parallel_per_row<<<count, numThreads>>>(d_in, d_out1, M, N);
  ...
  // launching the kernel
  transpose_parallel_per_element<<<blocks,threads>>>(d_in, d_out2,M,N,K1,K2);
